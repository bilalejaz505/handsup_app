...
class Install_core extends CI_Controller {
	...
	public function savedbsettings()
	{
		...
		else
		{
			...
			else if($this->check_db_connection()=="SUCCESS")
			{
				...
				$schema = read_file('./dbc_config/autocon.sql');
	...
	public function check_db_connection()
	{
		...
		$link = @mysqli_connect($this->input->post('db_host'),$this->input->post('db_user'),$this->input->post('db_pass'));
		if (!$link) {
		  @mysqli_close($link);
		  return "DBCONNFAIL";
		}
