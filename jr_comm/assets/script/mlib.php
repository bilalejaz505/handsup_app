...
/* Config file is generated by the installer, if it does not exist, redirect to installer */
if(!file_exists('mlib-config.php')){
header('location:mlib-install.php');
die();
}
...
$i=0;
$data = array();
$complete = mysqli_fetch_assoc(mysqli_query($mlib_db, "SELECT COUNT(*) as `gtotal` FROM `".MLIBPREFIX."uploads` WHERE `uid`='$mlib_current_user'"));
$qry = "SELECT * FROM `".MLIBPREFIX."uploads` WHERE `uid`='$mlib_current_user' ORDER BY `time` DESC limit $limit, $ipp";
$res = mysqli_query($mlib_db, $qry);

while($row = mysqli_fetch_assoc($res)){
$row['newtime'] = date("l, jS M Y, h:i:s a", $row['time']);
//$row['url'] = MLIBURL.$row['url'];
$data[] = $row;
++$i;
}
...
mlib_delete_file(MLIBPATH.'/mlib-uploads/full/'.$val.'.'.$data['type']);
mlib_delete_file(MLIBPATH.'/mlib-uploads/thumbs/w=150&h=150___'.$val.'.'.$data['type']);
