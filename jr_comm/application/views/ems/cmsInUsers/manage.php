	 ...
	 $('#published').change(function(event){
			...
			if($("td.blue_bar").length != 0) {
                        if (confirm('Are you sure to delete this?')) {
                            $("td.blue_bar").each(function() {
                                ...
                                var nxt_td = $('td#' + original_id).next().text(); // getting the name of User
                                var url = '<?php echo base_url(); ?>ems/cmsInUsers/checkUsers';
                                $.post(url, {id: id}, function(data) {
                                    if (data.trim() === '4') {
                                        alert("You cannot delete the user: '" + nxt_td + "' because this user is created from Central site.");
                                    } else {
        ...
        $('#edit').click( function() {
            ...
            if ($("td.blue_bar" ).length == 1) {
                ...
                $.post(url, {id: id}, function(data) {
                    if(data.trim() === '4'){
                        alert("You cannot edit this user because this user is created from Central site.");
                    }else{
...
<div class="inner_left_headingsmalll">Website</div>
              ...
              <div class="inner_mid_heading">CMS Users</div>
...
<span><img src="assets/images/black_bar.png"></span>
                ...
                <td width="70%" class="text_table_space">Username</td>
                <td width="20%" class="text_table_space">Role</td>
        	...
        	?>
                ...
                <!--  <td style="display:none"><?php echo $i; ?></td>-->
                   <td  class="black_bar" id="bar_<?php echo $val['id'] ?>"><?php echo $i; ?></td>
                    <td class="table_row"><?php echo $val['usr_uname']?></td>
                    <td class="table_row" style="border-right:#000 0px solid"><?php echo getUserRoleIn($val['usr_grp_id'])?></td>
                     <td class="table_row" style="border-right:#000 0px solid">
                         <?php echo get_grid_status($val['id'],$val['usr_pub_status']);?>
                     </td>                    
                </tr>
