	...
	$("#example tbody tr").click( function( e ) {
        ...
        $('#view_detail').click( function() {			
           ...
           // var temp = temp_id.split('_');
            ...
            if ($("td.blue_bar").length == 1 ) {
                window.location = '<?php echo base_url(); ?>ems/gallery/view/id/'+id+'/projects';
        		}else{
				alert("Select any one row from table");	
        ...
        $('#view_items').click( function() {			
           ...
           // var temp = temp_id.split('_');
            id = parseInt(temp[1]);
            if ($("td.blue_bar").length == 1 ) {
                window.location = '<?php echo base_url(); ?>ems/selectlist_items/manage/slid/'+id;
        		}else{
				alert("Select any one row from table");	
            ...
            if($("td.blue_bar" ).length!=0){
                if(confirm('Are you sure to delete this?')) {
                    $( "td.blue_bar" ).each(function() {
                        ...
                        id = parseInt(temp[1]);
                        $('#id_'+id).hide('slow');				 			 	
                        url ='<?php echo base_url(); ?>ems/projects/delete';
        ...
        $('#edit').click( function() {
            ...
            if($("td.blue_bar" ).length != 0)
            {
                ...
                if ($("td.blue_bar" ).length == 1 ) 
                {
                    window.location = '<?php echo base_url(); ?>ems/projects/edit/id/'+id;
                }else{
                    alert("Select any one row from table");	
                }
...
$('#example').tableDnD({
            onDrop: function(table, row) {
                ...
                var url = BASEPATH + 'ems/projects/sortCol?' + decodeURI($.tableDnD.serialize());
             ...
             <div class="inner_gray_panel" >
               ...
               <div class="side_right">
                 ...
                 <div class="previewChangespanel"> 
                                     ...
                                     <div class="previewChangesBtn"> <a href="<?php echo base_url(); ?>contactus" target="_blank"><img src="assets/images/previewChanges.png" width="108" height="23" border="0" alt=""/></a> </div>
                                     
                                     <div class="modifiedDate">Last modified <?php echo fetchLastupdated("selectlists","select_updated_at");?> </div>
...
to filter the grid</div>
    ...
    width: 100% !important;">
                   ...
                   <div class="filter_heading">Filters</div>
                    ...
                    <ul>
                    <li><a id="view_items">Select Options</a></li>
                    <?php echo top_navigation(0);?>
                    <li style="z-index:300; left:75%"><a href="ems/dashboard/manage">Back</a></li>
                    </ul>                     
                     </div><!--inner_menu_bar-->
        	...
        	?>
               ...
               <!-- <td style="display:none"><?php echo $count ?></td>-->
                   <td class="black_bar" id="bar_<?php echo $val['id'] ?>"><?php echo $count ?></td>
                    <td  class="table_row"><?php echo $val['eng_select_title']?></td>
                     <td  class="table_row"  style="border-right:#000 0px solid">
                     <?php echo get_grid_status($val['id'],$val['select_pub_status']);?>
                     </td> 
